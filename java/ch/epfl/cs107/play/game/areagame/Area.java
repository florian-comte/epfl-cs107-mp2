...
/**
 * Area is a "Part" of the AreaGame. An Area is made of a Behavior, and a List
 * of Actors
 */
public abstract class Area implements Playable, PauseMenu.Pausable {
	...
	/// List of Actors we want to register/unregistered from the area for next
	/// update iteration
	...
	/// - start indicate if area already begins, paused indicate if we display the
	/// pause menu
	...
	/**
	 * Setter for the Behavior of this Area Please call this method in the begin
	 * method of every subclass
	 * 
	...
	/**
	 * Add an actor to the actors list and to the behavior area cell if the actor is
	 * an Interactable and to the interactor list if the actor is an Interactor
	 * 
	 * @param a        (Actor)(Interactor?)(Interactable?): the actor to add, not
	 *                 null
	...
	/**
	 * Remove an actor form the actor list and from the behavior area cell if the
	 * actor is an Interactable and from the interactor list if the actor is an
	 * Interactor
	 * 
	...
	public final boolean registerActor(Actor a) {
		// TODO if actor can be registered: It is this Area decision, implement a
		// strategy
	...
	public final boolean unregisterActor(Actor a) {
		// TODO if actor can be unregistered: It is this Area decision, implement a
		// strategy
	...
	public Vector getRelativeMousePosition() {
		...
		/*
		 * .max(new Vector(0,0)) .min(new Vector(getWidth(),getHeight()));
		 */
	}
	...
	/**
	 * @return (boolean): true if the method begin already called once. You can use
	 *         resume() instead
	 */
	...
	public final boolean leaveAreaCells(Interactable entity, List<DiscreteCoordinates> coordinates) {
		// TODO if Interactable can leave the cells: It is this Area decision, implement
		// a strategy
		// Until now, the entity is put in a map waiting the update end to avoid
		// concurrent exception during interaction
	...
	public final boolean enterAreaCells(Interactable entity, List<DiscreteCoordinates> coordinates) {
		// TODO if Interactable can enter the cells: It is this Area decision, implement
		// a strategy
		// Until now, the entity is put in a map waiting the update end to avoid
		// concurrent exception during interaction
	...
	/**
	 * Can be called by any possessor of this Area. Caller indicate it requests a
	 * pause with given menu displayed. Notice: this method chooses if the request
	 * ends up or not
	 * 
	 * @param menu (AreaPauseMenu): The context menu to display. It (or any of its
	 *             components) will be responsible of the ResumeRequest, not null
	 */
	public final void requestAreaPause(AreaPauseMenu menu) {
		...
		if (menu != null) {
			this.menu = menu;
			// Important to begin the menu each time : isResumeRequested must be set to
			// false
	...
	/**
	 * Can be called by anny possessor of this Area Caller indicates it requests a
	 * resume of the pause state to the game Notice: this method chooses if the
	 * request ends up or not
	 */
	@Override
