...
public class ICWarsInfoPanel implements Graphics {
	...
	public ICWarsInfoPanel(float cameraScaleFactor) {
		...
		citySprite = new ImageGraphics(ResourcePath.getSprite("icwars/neutralBuilding"), 1f, 1f, null, anchor, 1f,
				3001f);
		...
		cellTypeText = new TextGraphics("", fontSize, Color.WHITE, null, 0.0f, false, false, new Vector(0, -0.3f),
				TextAlign.Horizontal.LEFT, TextAlign.Vertical.MIDDLE, 1.0f, 3001f);

		cellDefenseText = new TextGraphics("", fontSize, Color.WHITE, null, 0.0f, false, false,
				new Vector(0, -2.5f * fontSize - 0.7f), TextAlign.Horizontal.LEFT, TextAlign.Vertical.MIDDLE, 1.0f,
				3001f);

		unitNameText = new TextGraphics("", fontSize, Color.WHITE, null, 0.0f, false, false, new Vector(0, -0.3f),
				TextAlign.Horizontal.LEFT, TextAlign.Vertical.MIDDLE, 1.0f, 3001f);

		unitHealthText = new TextGraphics("", fontSize, Color.WHITE, null, 0.0f, false, false,
				new Vector(0, -1.25f * fontSize - 0.5f), TextAlign.Horizontal.LEFT, TextAlign.Vertical.MIDDLE, 1.0f,
				3001f);

		unitDamageText = new TextGraphics("", fontSize, Color.WHITE, null, 0.0f, false, false,
				new Vector(0, -2.5f * fontSize - 0.4f), TextAlign.Horizontal.LEFT, TextAlign.Vertical.MIDDLE, 1.0f,
				3001f);
	}
	...
	public void setUnit(ICWarsUnit unit) {
		this.unit = unit;
	}
	...
	private void drawCellDetails(Canvas canvas, float height, float width) {
		...
		final Transform spriteTransform = Transform.I
	...
	private void drawUnitDetails(Canvas canvas, float height, float width) {
		...
		final Transform characteristicsTransform = Transform.I
